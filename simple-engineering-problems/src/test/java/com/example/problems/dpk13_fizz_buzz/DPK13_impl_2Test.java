...
class DPK13_impl_2Test {
    ...
    void fizzBuzz100ShouldBeSuccess() {
        ...
        Object[] result = DPK13_impl_2.fizzBuzz(100);

        Assertions.assertEquals(expected.length, result.length);

        for (int i = 0; i < expected.length; i++) {
            Assertions.assertEquals(expected[i].toString(), result[i].toString());
        }
    }
    ...
    void fizzBuzz10ShouldBeSuccess() {
        ...
        Object[] result = DPK13_impl_2.fizzBuzz(10);
        Assertions.assertEquals(expected.length, result.length);
        for (int i = 0; i < expected.length; i++) {
            Assertions.assertEquals(expected[i].toString(), result[i].toString());
        }
